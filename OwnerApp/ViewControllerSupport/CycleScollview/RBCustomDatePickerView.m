...
#import "MXSConferenceDatePickerDelegate.h"
...
- (void)setTimeBroadcastView
{
    ...
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月%d日 %@ %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],(int)monthInt,(int)dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    [self addSubview:nowPickerShowTimeLabel];
    
    timeBroadcastView = [[UIView alloc] initWithFrame:CGRectMake(20, 140, 278.5, 190.0)];
//    timeBroadcastView.layer.cornerRadius = 2;//设置视图圆角
//    timeBroadcastView.layer.masksToBounds = YES;
    ...
    timeBroadcastView.layer.borderWidth = .5;
    [self addSubview:timeBroadcastView];
    UIView *beforeSepLine = [[UIView alloc] initWithFrame:CGRectMake(0, 39, 278.5, 0.5)];
    ...
    [middleSepView setBackgroundColor:[UIColor colorWithRed:0.338 green:0.634 blue:1.000 alpha:1.000]]; //RGBA(249.0, 138.0, 20.0, 1.0)
    [timeBroadcastView addSubview:middleSepView];
    UIView *bottomSepLine = [[UIView alloc] initWithFrame:CGRectMake(0, 150.5, 278.5, 0.5)];
    ...
    selectTimeIsNotLegalLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 80, 278.5, 15)]; // 339.5 + 40
    ...
    OkBtn.frame = CGRectMake(20, 339.5, 278.5, 45);
    OkBtn.layer.borderColor = [UIColor colorWithRed:221.0/255.0 green:221.0/255.0 blue:221.0/255.0 alpha:1.0].CGColor;
    OkBtn.layer.borderWidth = .5f;
    OkBtn.layer.masksToBounds = YES;
    OkBtn.backgroundColor = [UIColor clearColor];
    [OkBtn setTitle:@"确定选择" forState:UIControlStateNormal];
    OkBtn.titleLabel.font = [UIFont systemFontOfSize:15.0f];
    [OkBtn setTitleColor:[UIColor colorWithWhite:0.507 alpha:1.000] forState:UIControlStateNormal];
    [OkBtn setBackgroundImage:[self buttonImageFromColor:[UIColor colorWithWhite:0.767 alpha:1.000]] forState:UIControlStateHighlighted];
...
-(IBAction)selectDate:(id)sender
{
    ...
    if([_delegate respondsToSelector:@selector(conferenceDatePicker:saveDate:)])
        [_delegate conferenceDatePicker:self saveDate:currentSelectDate];
...
- (void)selectSetBroadcastTime
{
    ...
    NSString *taskDateString = [NSString stringWithFormat:@"%d%02d%02d%02d%02d%02d",(int)yearInt,(int)monthInt,(int)dayInt,(int)hourInt,(int)minuteInt,(int)secondInt];
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSInteger minuteInt = minuteLabel.tag - 1;
    NSInteger secondInt = secondLabel.tag - 1;
    NSString *dateString = [NSString stringWithFormat:@"%d%02d%02d%02d%02d%02d",(int)yearInt,(int)monthInt,(int)dayInt,(int)hourInt,(int)minuteInt,(int)secondInt];
    NSString *weekString = [self fromDateToWeek:dateString];
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月%d日 %@ %02d:%02d:%02d",(int)yearInt,(int)monthInt,(int)dayInt, weekString,(int)hourInt,(int)minuteInt,(int)secondInt];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString *selectTimeString = [NSString stringWithFormat:@"%d-%02d-%02d %02d:%02d:%02d",(int)yearInt,(int)monthInt,(int)dayInt,(int)hourInt,(int)minuteInt,(int)secondInt];
    ...
    currentSelectDate = selectDate;
     if (NSOrderedAscending >= [selectDate compare:nowStrDate]) {//选择的时间与当前系统时间做比较
        [selectTimeIsNotLegalLabel setTextColor:[UIColor colorWithRed:1.000 green:0.134 blue:0.186 alpha:1.000]];//RGBA(155, 155, 155, 1)
        selectTimeIsNotLegalLabel.text = @"温馨提示：所选时间不能是当前时间之前";
        [OkBtn setEnabled:NO];
    }
    else
    {
